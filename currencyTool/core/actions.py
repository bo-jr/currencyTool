    ...
    def __call__(self, parser, namespace, value, option_strings=None):
        ...
        try:
            with DbClient('exchange_rates', 'config') as db:
                db.update(
                    {'base_currency': {'$ne': None}}, # update item in collection where base currency is not equal to None
                    {'baes_currency': value}
                )
...
class UpdateForeignerExchangeRates(Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        ...
        try:
            ...
            print(('Fetching exchange rates from fixer.io' f' [base currency: {base_currency}]'))
